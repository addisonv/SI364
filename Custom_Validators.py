...
#If, in the example above, the name field were to be split into two field for first name 
...
#What is done above is we have moded the exact same code out of the class and as a function
#Since a validator can be any callable which accpets the two positional arguments form and field
# this is perfely fine, but the validator is very special-cased.

#Instead, we can turn out validator into a more powerful one by making it a factory which returns a callable 
...
#In addition to allowing the error message to be customized, we've now converted the length validator
